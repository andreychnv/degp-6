...
public class AmountAlertsApp {
    ...
    public static Topology buildTopology(SchemaRegistryClient client, Map<String, String> serDeConfig) {
        ...
        var purchaseWithJoinedProduct = purchasesStream.leftJoin(
                ...
                AmountAlertsApp::joinProduct // указываем, как джоинить
        );
        ...
        purchaseWithJoinedProduct
                ...
                .aggregate(
                        () -> 0L,
                        (key, val, agg) -> agg += (Long) val.get("purchase_quantity") * val.get("product_price"),
    ...
    private static AmountAlertsApp.JoinResult joinProduct(GenericRecord purchase, GenericRecord product) {
        try {
            ...
            return new AmountAlertsApp.JoinResult(true, result, null);
        } catch (Exception e) {
            return new AmountAlertsApp.JoinResult(false, purchase, e.getMessage());
        }
    }
